...
const bannerDesigns = [
	...
	// "profile-banner-christmas",
];

// const user = JSON.parse(localStorage.getItem("user-info"));
// const banner = user?.user_creds?.profile_banner;
...
function ProfileChangeBannerModal({
	...
	// const initialBanner = bannerDesigns.includes(banner)
	// ? banner
	// : bannerDesigns[0];

	const [selectedBanner, setSelectedBanner] = useState("");
	const { postUpdateProfileBanner } = useUser();
	const { isMobile } = useDeviceSize();
	const [isBannerLoading, setIsBannerLoading] = useState(false);

	const handleUpdateProfileBanner = async () => {
		await postUpdateProfileBanner(
			email,
			selectedBanner,
			setIsBannerLoading
		);
